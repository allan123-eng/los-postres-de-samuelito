...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#F8F7FF] p-4 text-gray-900">
      ...
      <input 
        ...
        className="w-full max-w-6xl mx-auto block p-5 rounded-2xl mb-8 border-2 border-purple-50 outline-none text-gray-900 bg-white font-bold"
            ...
            <div className="p-8">
              ...
              <div className="flex justify-between items-center">
                ...
                <button 
                  ...
                  className="w-12 h-12 rounded-2xl bg-purple-600 text-white font-bold text-2xl shadow-lg active:scale-95">+</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* CARRITO LATERAL (ARREGLADO) */}
      {isCartOpen && (
        <>
          <div className="fixed inset-0 bg-purple-900/20 backdrop-blur-md z-[200]" onClick={() => setIsCartOpen(false)} />
          <div className="fixed right-0 top-0 h-full w-full max-w-sm bg-white z-[210] p-8 shadow-2xl flex flex-col">
                ...
                carrito.map((item) => (
                  <div key={item.id} className="flex justify-between items-center bg-purple-50/50 p-4 rounded-2xl border border-purple-100">
                    ...
                    <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-purple-100">
                      <button onClick={() => restarDelCarrito(item.id)} className="w-6 h-6 flex items-center justify-center text-gray-400 font-bold">-</button>
            ...
            {carrito.length > 0 && (
              <div className="mt-8 pt-8 border-t border-purple-100">
                ...
                <button onClick={enviarPedido} className="w-full py-5 rounded-3xl font-black text-xl bg-green-500 text-white shadow-xl shadow-green-100">Confirmar WhatsApp 💬</button>
              </div>
            )}
          </div>
        </>
      ...
      {/* PANEL ADMIN (LETRAS NEGRAS ARREGLADAS) */}
      <button onClick={() => setIsAdminOpen(true)} className="fixed bottom-6 right-6 w-12 h-12 bg-white shadow-xl rounded-full z-50 opacity-20 hover:opacity-100">⚙️</button>
      
      {isAdminOpen && (
        <div className="fixed inset-0 bg-purple-900/60 backdrop-blur-lg z-[250] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-8 max-h-[90vh] overflow-y-auto text-gray-900">
            {!isAutenticado ? (
              <div className="text-center space-y-6">
                <h2 className="text-2xl font-black text-gray-900">Acceso Admin 🔐</h2>
                <input type="password" placeholder="Ingresa la clave" className="w-full p-5 rounded-2xl border-2 border-purple-200 text-center text-gray-900 font-black text-xl bg-white" onChange={(e) => setPasswordInput(e.target.value)} />
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b pb-4">
                  <h2 className="text-2xl font-black text-gray-900 uppercase italic">Editor 👩‍🍳</h2>
                ...
                <form onSubmit={gestionarGuardar} className="space-y-4 bg-purple-50 p-6 rounded-[2rem] border-2 border-purple-500">
                  <div className="space-y-1">
                    ...
                    <input name="nombre" placeholder="Nombre" defaultValue={editando?.nombre} className="w-full p-4 rounded-xl border-2 border-purple-100 text-gray-900 font-bold bg-white" required />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="file" accept="image/*" onChange={manejarImagen} className="w-full p-3 rounded-xl bg-white border-2 border-purple-100 text-gray-900 font-bold text-sm" />
                  </div>
                  <button className="w-full bg-purple-600 text-white py-5 rounded-2xl font-black text-xl shadow-lg uppercase">
                  ...
                  {productos.map(p => (
                    <div key={p.id} className="flex justify-between items-center bg-white border-2 border-purple-50 p-3 rounded-2xl">
                      <div className="flex items-center gap-3">
                        <img src={p.img} className="w-12 h-12 rounded-lg object-cover" alt="mini" />
